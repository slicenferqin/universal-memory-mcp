...
async function benchmark() {
  ...
  duration = Date.now() - start
  console.log(
    `   ✅ With cache (cold): ${duration}ms (${(duration / 10).toFixed(1)}ms per embedding)\n`
  )
  ...
  duration = Date.now() - start
  console.log(
    `   🚀 With cache (warm): ${duration}ms (${(duration / 10).toFixed(1)}ms per embedding)\n`
  )

  const cacheStats = cachedProvider.getCacheStats()
  console.log(
    `   Cache stats: ${cacheStats.hitRate.toFixed(1)}% hit rate (${cacheStats.hits}/${cacheStats.hits + cacheStats.misses})\n`
  )
  ...
  duration = Date.now() - start
  console.log(
    `   Batch (no cache): ${duration}ms (${(duration / 10).toFixed(1)}ms per embedding)\n`
  )
  ...
  if (stats.totalChunks > 0) {
    ...
    for (const limit of limits) {
      ...
      duration = Date.now() - start
      console.log(`   Limit ${String(limit).padStart(2)}: ${duration}ms`)
    }
    console.log('')
  }
